...
4. **KTCL-K8s** - Kubernetes Job実行クライアント（タスクをK8s Jobとして実行）
5. **KTCL-ClaudeCode** - Claude Code統合クライアント
6. **KTCL-Web** - Ktorベースのクライアントウェブアプリケーション（レガシー）
7. **Kodel** - 共通ライブラリ（API、コア、コルーチン）
8. **Keruta SDK** - クライアントSDK（別リポジトリ）
