...
export class WebSocketService {
  ...
  constructor(options: WsConnectionOptions) { this.options = options; }

  connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      if (this.connectionState === "connected") { resolve(); return; }
      ...
      this.websocket.onclose = () => { this.connectionState = "disconnected"; this.options.onClose?.(); };
      this.websocket.onerror = (error) => { this.connectionState = "error"; this.options.onError?.(error); reject(error); };
  ...
  getState(): WsConnectionState { return this.connectionState; }
