...
class KubernetesClientProvider(
    ...
    /**
     * Gets or creates the keruta-agent-token secret in the specified namespace.
     * This token is used for agent authentication with the WebSocket API.
     *
     * @param namespace The namespace to get or create the secret in
     * @return The token value, or null if creation failed
     */
    fun getOrCreateAgentTokenSecret(namespace: String): String? {
        val secretName = "keruta-agent-token"
        ...
        if (secretExists(secretName, namespace)) {
            ...
            try {
                ...
                if (existingSecret.data != null && existingSecret.data.containsKey("token")) {
                    val encodedToken = existingSecret.data["token"]
                    if (encodedToken != null) {
                        val token = String(Base64.getDecoder().decode(encodedToken))
                        return token
                    }
                }
            } catch (e: Exception) {
                logger.error("Failed to retrieve existing agent token from secret", e)
            }

            // If we couldn't retrieve the token, generate a new one
            return updateAgentTokenSecret(namespace)
        }

        // Generate a JWT token for agent access
        val token = jwtTokenProvider.createApiToken("keruta-agent")
        ...
        return if (createdSecret != null) {
            logger.info("Created keruta-agent-token secret in namespace $namespace")
            token
        } else {
            logger.error("Failed to create keruta-agent-token secret in namespace $namespace")
            null
        }
    }

    /**
     * Updates the keruta-agent-token secret in the specified namespace with a new token.
     * If the secret doesn't exist, it will be created.
     *
     * @param namespace The namespace to update or create the secret in
     * @return The new token value, or null if update/creation failed
     */
    fun updateAgentTokenSecret(namespace: String): String? {
        val secretName = "keruta-agent-token"

        try {
            val client = getClient() ?: return null

            // Generate a new JWT token for agent access
            val token = jwtTokenProvider.createApiToken("keruta-agent")
            ...
            if (secretExists(secretName, namespace)) {
                ...
                // Update the secret in Kubernetes
                client.secrets().inNamespace(namespace).withName(secretName).replace(existingSecret)
                logger.info("Updated keruta-agent-token secret in namespace $namespace")
                return token
            } else {
                // Create a new secret
                val data = mapOf("token" to token)
            ...
            logger.error("Failed to update keruta-agent-token secret in namespace $namespace", e)
            return null
        }
    }
}
