...
graph TB
    subgraph "Kerutaシステム"
        subgraph "フロントエンド層"
            ktcl-front["ktcl-front<br/>(React + Keycloak.js)"]
            kicl-web["kicl-web<br/>(React Router v7)"]
        end

        subgraph "アプリケーション層"
            ktse["ktse<br/>(Ktorタスクサーバー)<br/>Exposed + Flyway + MySQL"]
            ktcl-k8s["ktcl-k8s<br/>(K8s実行エンジン)<br/>OIDC + PKCE"]
            kise["kise<br/>(ID Server)<br/>開発中"]
        end
...
| **kicl-web** | ユーザー完了のインターフェースモジュール（React Router v7 + KMP統合） | HTTP/REST |
...
├── kicl-web/               # ユーザー完了のインターフェースモジュール（React Router v7 + KMP）
...
### 9.2 kicl-web（ユーザー完了のインターフェースモジュール）

Kerutaの最終的なユーザーインターフェース。ユーザーが実際に利用する完成形のWebアプリケーション。KMP（Kotlin Multiplatform）モジュールと統合され、サーバーサイドロジックをクライアントで再利用可能。

詳細は [kicl-web.md](kicl-web.md) を参照。

| 項目 | 内容 |
|---|---|
| 位置付け | ユーザー完了のインターフェース（最終UI） |
...
| `Dockerfile_kicl_web` | `harbor.kigawa.net/private/kicl-web` | ユーザー完了のインターフェース（nginx） |
