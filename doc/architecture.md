...
graph TB
    subgraph "Kerutaシステム"
        ...
        subgraph "アプリケーション層"
            ...
            kise["kise<br/>(ID Server)<br/>ID管理・認証基盤"]
        end
...
| **kicl-web** | ユーザー管理のインターフェースモジュール（React Router v7 + KMP統合） | kise/kicp 経由 |
| **kise** | Keruta専用IDサーバー（ID管理・認証基盤） | HTTP, WebSocket |
...
ユーザー管理画面を提供するWebアプリケーション。**kise（IDサーバー）** と **kicp（クロスドメインID連携）** を通じて通信し、ユーザーのID・アカウント情報を管理する。KMP（Kotlin Multiplatform）モジュールと統合され、サーバーサイドロジックをクライアントで再利用可能。
...
| 通信先 | kise（ID管理） + kicp（クロスドメインID連携） |
