...
```bash
# プロジェクトルートで実行
docker build -f Dockerfile_ktcl_k8s -t harbor.kigawa.net/library/keruta-ktcl-k8s:latest .
```
...
# イメージをプッシュ
docker push harbor.kigawa.net/library/keruta-ktcl-k8s:latest

# タグ付けしてバージョン管理
docker tag harbor.kigawa.net/library/keruta-ktcl-k8s:latest harbor.kigawa.net/library/keruta-ktcl-k8s:v1.0.0
docker push harbor.kigawa.net/library/keruta-ktcl-k8s:v1.0.0
```
...
docker buildx build --platform linux/amd64,linux/arm64 \
  ...
  -t harbor.kigawa.net/library/keruta-ktcl-k8s:latest \
  --push .
```
