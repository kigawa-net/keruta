...
すべての開発者は、以下に定める規約および `doc/` ディレクトリ内の各種規約を遵守しなければならない。

## 0. 実装開始前の必須フロー

**実装開始前に以下を順番通りに実施すること。**

### 0-1. Issue 作成

- ブランチ作成・実装開始の前に、必ず GitHub Issue を作成する
- Issue には「目的」「作業計画」「影響範囲」「完了条件」を記載する
- 詳細は [`doc/issue-convention.md`](doc/issue-convention.md) を参照

### 0-2. 実装計画PRの作成・マージ

- `docs/{module}-{feature}-plan` ブランチを作成し、`doc/plan/{module}-{feature}.md` に実装計画書を記述する
- 計画書には「実装方針」「ファイル・クラス構成」「実装順序」「テスト方針」を記載する
- `develop` ベースでPRを作成し、マージされてから実装ブランチを作成する
